          ...
          <div class="sm:rounded-lg">
            <div class="p-6 sm:rounded-t-lg border-b border-gray-200 dark:border-gray-700 block md:flex md:items-center md:justify-between">
              ...
              <div class="mt-4 sm:mt-0 block sm:flex sm:items-center sm:gap-4">
                <Button :href="route('customers.create')">{{ t('Add New Customer') }}</Button>
            ...
            <div class="bg-gray-50 dark:bg-black/50 border-b dark:border-gray-700 sm:rounded-b-lg">
              <div v-if="props.customers && props.customers.data && props.customers.data.length">
                <div class="flex flex-col">
                        ...
                        <table class="min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700">
                            ...
                            <tr>
                              <th scope="col" class="relative w-12 px-6 sm:w-16 sm:px-8">
                                <input
                                  ...
                                  :checked="indeterminate || form.selection.length === props.customers.data.length"
                                  @change="form.selection = $event.target.checked ? props.customers.data.map(p => p.id) : []"
                              ...
                              <th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">
                                {{ t($settings?.customer_cf1 || 'Custom Field 1') }}
                              </th>
                              <th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">
                                {{ t($settings?.customer_cf2 || 'Custom Field 2') }}
                              </th>
                          ...
                          <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                            ...
                            >
                              ...
                              <td class="whitespace-nowrap px-3 py-4 text-sm font-medium">{{ customer.name }}</td>
                              <td class="whitespace-nowrap px-3 py-4 text-sm">{{ customer.company }}</td>
                              <td class="whitespace-nowrap px-3 py-4 text-sm">{{ customer.email }}</td>
                              <td class="whitespace-nowrap px-3 py-4 text-sm">{{ customer.phone }}</td>
                              <td class="whitespace-nowrap px-3 py-4 text-sm">{{ customer.cf1 }}</td>
                              <td class="whitespace-nowrap px-3 py-4 text-sm">{{ customer.cf2 }}</td>
                  ...
                  <pagination class="m-4" :meta="props.customers.meta" :links="props.customers.links" />
                </div>
              </div>
