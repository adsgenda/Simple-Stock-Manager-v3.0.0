...
const submit = () => {
  ...
  // this.$router.post(route('customers.import.save'), data);
  form.post(route('customers.import.save'), { preserveScroll: true });
};
</script>

<template>
  <AppLayout :title="t('Import {x}', { x: t('Customers') })">
      ...
      <FormSection @submitted="submit">
        ...
        <template #description>
          ...
          <div class="flex items-center gap-4 flex-wrap mt-4">
            <Button :href="route('customers.index')">{{ t('List Customers') }}</Button>
            <Button away :href="route('customers.export')">{{ t('Export Customers') }}</Button>
          </div>
        </template>
          ...
          <div class="w-full col-span-full">
            ...
            >
              <div class="space-y-1 text-center">
                ...
                <div class="text-xs">
                  <div>{{ t('Excel file should have name, company, phone, email, cf1 and cf2 columns.') }}</div>
                  <div>{{ t('The name and phone or email are required.') }}</div>
                </div>
