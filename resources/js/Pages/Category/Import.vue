...
const submit = () => {
  ...
  // this.$router.post(route('categories.import.save'), data);
  form.post(route('categories.import.save'), { preserveScroll: true });
};
</script>

<template>
  <AppLayout :title="t('Import {x}', { x: t('Categories') })">
      ...
      <FormSection @submitted="submit">
        ...
        <template #description>
          ...
          <div class="flex items-center gap-4 flex-wrap mt-4">
            <Button :href="route('categories.index')">{{ t('List Categories') }}</Button>
            <Button away :href="route('categories.export')">{{ t('Export Categories') }}</Button>
          </div>
        </template>
          ...
          <div class="w-full col-span-full">
            ...
            >
              <div class="space-y-1 text-center">
                ...
                <div class="text-xs">
                  <div>{{ t('Excel file should have name and code columns both required.') }}</div>
                  <!-- <div>{{ t('You must fill the name and code columns.') }} {{ t('parent is the code of the parent category.') }}</div> -->
                </div>
