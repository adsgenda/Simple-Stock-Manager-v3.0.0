...
import _sfc_main$b from "./Details-D-k2JSVu.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const deleteRow = (id, force = false) => {
      router.delete(T("checkouts.destroy" + (force ? ".permanently" : ""), id));
    };
    const restoreRow = (id) => {
      router.put(T("checkouts.restore", id));
    };
    const closeModal = () => {
    ...
    const deleteSelected = (force) => {
      form.delete(T("checkouts.destroy.many"), {
        onSuccess: () => {
          form.reset();
        },
        onFinish: () => {
    ...
    const searchNow = () => {
      ...
      }).get(T("checkouts.index"), { preserveState: true });
    };
    const resetSearch = () => {
      search.search = "";
      search.trashed = null;
      ...
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (props.checkouts && props.checkouts.data && props.checkouts.data.length) {
              ...
              _push2(`<table class="min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700"${_scopeId}><thead class="bg-gray-50 dark:bg-black/50"${_scopeId}><tr${_scopeId}><th scope="col" class="relative w-12 px-6 sm:w-16 sm:px-8"${_scopeId}><input type="checkbox"${ssrRenderAttr("indeterminate", indeterminate.value)}${ssrIncludeBooleanAttr(indeterminate.value || unref(form).selection.length === props.checkouts.data.length) ? " checked" : ""} class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 sm:left-6 dark:bg-gray-700 dark:border-gray-700"${_scopeId}></th><th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"${_scopeId}>${ssrInterpolate(unref(t)("Date"))}</th><th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"${_scopeId}>${ssrInterpolate(unref(t)("Reference"))}</th><th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"${_scopeId}>${ssrInterpolate(unref(t)("Customer"))}</th><th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"${_scopeId}>${ssrInterpolate(unref(t)("Items"))}</th><th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 w-12"${_scopeId}></th><th scope="col" class="relative py-3.5 pl-3 pr-4 sm:pr-6"${_scopeId}><span class="sr-only"${_scopeId}>${ssrInterpolate(unref(t)("Actions"))}</span></th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900"${_scopeId}><!--[-->`);
              ssrRenderList(props.checkouts.data, (checkout2) => {
                var _a;
                _push2(`<tr class="${ssrRenderClass([
                  unref(form).selection.includes(checkout2.id) && "bg-gray-50 dark:bg-gray-800/50",
                  checkout2.deleted_at && "bg-red-100 dark:bg-red-900"
                ])}"${_scopeId}><td class="relative w-12 px-6 sm:w-16 sm:px-8"${_scopeId}>`);
                if (unref(form).selection.includes(checkout2.id)) {
                  _push2(`<div class="absolute inset-y-0 left-0 w-0.5 bg-blue-600"${_scopeId}></div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`<input type="checkbox"${ssrRenderAttr("value", checkout2.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).selection) ? ssrLooseContain(unref(form).selection, checkout2.id) : unref(form).selection) ? " checked" : ""} class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 sm:left-6 dark:bg-gray-700 dark:border-gray-700"${_scopeId}></td><td class="whitespace-nowrap px-3 py-4 cursor-pointer text-sm font-medium"${_scopeId}>${ssrInterpolate(_ctx.$date(checkout2.date))}</td><td class="whitespace-nowrap px-3 py-4 cursor-pointer text-sm"${_scopeId}>${ssrInterpolate(checkout2.reference)}</td><td class="whitespace-nowrap px-3 py-4 cursor-pointer text-sm"${_scopeId}>${ssrInterpolate((_a = checkout2.customer) == null ? void 0 : _a.name)}</td><td class="whitespace-nowrap px-3 py-4 cursor-pointer text-sm text-right w-20"${_scopeId}>${ssrInterpolate(checkout2.items.length)}</td><td class="whitespace-nowrap px-3 py-4 text-sm w-12"${_scopeId}><div class="flex items-center justify-end gap-3 -my-1"${_scopeId}>`);
                if (checkout2.attachment) {
                  _push2(`<a${ssrRenderAttr("href", checkout2.attachment)} target="_blank" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244"${_scopeId}></path></svg></a>`);
                } else {
                  _push2(`<!---->`);
                }
                ...
                _push2(ssrRenderComponent(_sfc_main$8, {
                  ...
                  editStr: unref(T)("checkouts.edit", checkout2.id)
              ...
              _push2(ssrRenderComponent(_sfc_main$9, {
                class: "m-4",
                meta: props.checkouts.meta,
                links: props.checkouts.links
              }, null, _parent2, _scopeId));
              _push2(`</div></div>`);
            } else {
            ...
            _push2(ssrRenderComponent(_sfc_main$a, {
              ...
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  ...
                  if (checkout.value) {
                    _push3(ssrRenderComponent(_sfc_main$b, { checkout: checkout.value }, null, _parent3, _scopeId2));
                  } else {
                    _push3(`<!---->`);
                  }
                  _push3(`</div></div>`);
                  ...
                  return [
                    createVNode("div", { class: "px-6 pt-3 pb-6 print:px-0 bg-gray-100 dark:bg-gray-800" }, [
                      show.value && checkout.value ? (openBlock(), createBlock("div", {
                        key: 0,
                        class: "flex items-center justify-between print:hidden"
                      }, [
                        createVNode("div", { class: "text-lg" }, [
                          createTextVNode(toDisplayString(unref(t)("Checkout Details")) + " ", 1),
                          createVNode("span", { class: "hidden sm:inline" }, "(" + toDisplayString(checkout.value.reference) + ")", 1)
                        ]),
                        createVNode("div", { class: "-mr-2 flex items- gap-2" }, [
                          ...
                          _ctx.$can("update-checkouts") ? (openBlock(), createBlock(_component_Link, {
                            key: 0,
                            href: unref(T)("checkouts.edit", checkout.value.id),
                            class: "flex items-center justify-center h-8 w-8 rounded-full text-gray-600 hover:text-gray-800 hover:bg-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-700 focus:outline-none"
                          }, {
                      ...
                      createVNode("div", { class: "" }, [
                        checkout.value ? (openBlock(), createBlock(_sfc_main$b, {
                          key: 0,
                          checkout: checkout.value
                        }, null, 8, ["checkout"])) : createCommentVNode("", true)
                      ])
                    ])
                  ];
                }
                    ...
                    createVNode("div", { class: "bg-white dark:bg-gray-900 shadow sm:rounded-lg" }, [
                      createVNode("div", { class: "sm:rounded-lg" }, [
                        ...
                        createVNode("div", { class: "bg-gray-50 dark:bg-black/50 border-b dark:border-gray-700 sm:rounded-b-lg" }, [
                          props.checkouts && props.checkouts.data && props.checkouts.data.length ? (openBlock(), createBlock("div", { key: 0 }, [
                                  ...
                                  createVNode("div", { class: "relative overflow-hidden ring-1 ring-black ring-opacity-5" }, [
                                    ...
                                    createVNode("table", { class: "min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700" }, [
                                        ...
                                        createVNode("tr", null, [
                                          createVNode("th", {
                                            ...
                                            createVNode("input", {
                                              ...
                                              checked: indeterminate.value || unref(form).selection.length === props.checkouts.data.length,
                                              onChange: ($event) => unref(form).selection = $event.target.checked ? props.checkouts.data.map((p) => p.id) : [],
                                          ...
                                          createVNode("th", {
                                            scope: "col",
                                            class: "py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"
                                          }, toDisplayString(unref(t)("Customer")), 1),
                                          createVNode("th", {
                                            scope: "col",
                                            class: "py-3.5 px-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"
                                      ...
                                      createVNode("tbody", { class: "divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900" }, [
                                        (openBlock(true), createBlock(Fragment, null, renderList(props.checkouts.data, (checkout2) => {
                                          var _a;
                                          return openBlock(), createBlock("tr", {
                                            ...
                                            createVNode("td", { class: "relative w-12 px-6 sm:w-16 sm:px-8" }, [
                                              unref(form).selection.includes(checkout2.id) ? (openBlock(), createBlock("div", {
                                                key: 0,
                                            ...
                                            createVNode("td", {
                                              ...
                                              onClick: ($event) => showCheckout(checkout2)
                                            }, toDisplayString(_ctx.$date(checkout2.date)), 9, ["onClick"]),
                                            createVNode("td", {
                                              class: "whitespace-nowrap px-3 py-4 cursor-pointer text-sm",
                                              onClick: ($event) => showCheckout(checkout2)
                                            }, toDisplayString(checkout2.reference), 9, ["onClick"]),
                                            createVNode("td", {
                                              class: "whitespace-nowrap px-3 py-4 cursor-pointer text-sm",
                                              onClick: ($event) => showCheckout(checkout2)
                                            }, toDisplayString((_a = checkout2.customer) == null ? void 0 : _a.name), 9, ["onClick"]),
                                            createVNode("td", {
                                              class: "whitespace-nowrap px-3 py-4 cursor-pointer text-sm text-right w-20",
                                              onClick: ($event) => showCheckout(checkout2)
                                            }, toDisplayString(checkout2.items.length), 9, ["onClick"]),
                                            createVNode("td", { class: "whitespace-nowrap px-3 py-4 text-sm w-12" }, [
                                              createVNode("div", { class: "flex items-center justify-end gap-3 -my-1" }, [
                                                checkout2.attachment ? (openBlock(), createBlock("a", {
                                                  key: 0,
                                                  href: checkout2.attachment,
                                                  target: "_blank",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Checkout/List.vue");
