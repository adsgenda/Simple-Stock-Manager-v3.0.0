...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const deleteRow = (id, force = false) => {
      router.delete(T("roles.destroy" + (force ? ".permanently" : ""), id));
    };
    const restoreRow = (id) => {
      router.put(T("roles.restore", id));
    };
    const closeModal = () => {
    ...
    const deleteSelected = (force) => {
      form.delete(T("roles.destroy.many"), {
        onSuccess: () => {
          form.reset();
        },
        onFinish: () => {
    ...
    const searchNow = () => {
      ...
      }).get(T("roles.index"), { preserveState: true });
    };
    const resetSearch = () => {
      search.search = "";
      search.trashed = null;
      ...
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (props.roles && props.roles.data && props.roles.data.length) {
              ...
              _push2(`<table class="min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700"${_scopeId}><thead class="bg-gray-50 dark:bg-black/50"${_scopeId}><tr${_scopeId}><th scope="col" class="relative w-12 px-6 sm:w-16 sm:px-8"${_scopeId}><input type="checkbox"${ssrRenderAttr("indeterminate", indeterminate.value)}${ssrIncludeBooleanAttr(indeterminate.value || unref(form).selection.length === props.roles.data.length) ? " checked" : ""} class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 sm:left-6 dark:bg-gray-700 dark:border-gray-700"${_scopeId}></th><th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100"${_scopeId}>${ssrInterpolate(unref(t)("Name"))}</th><th scope="col" class="relative py-3.5 pl-3 pr-4 sm:pr-6"${_scopeId}><span class="sr-only"${_scopeId}>${ssrInterpolate(unref(t)("Actions"))}</span></th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900"${_scopeId}><!--[-->`);
              ssrRenderList(props.roles.data, (role) => {
                _push2(`<tr class="${ssrRenderClass([
                  unref(form).selection.includes(role.id) && "bg-gray-50 dark:bg-gray-800/50",
                  role.deleted_at && "bg-red-100 dark:bg-red-900"
                ])}"${_scopeId}><td class="relative w-12 px-6 sm:w-16 sm:px-8"${_scopeId}>`);
                if (unref(form).selection.includes(role.id)) {
                  _push2(`<div class="absolute inset-y-0 left-0 w-0.5 bg-blue-600"${_scopeId}></div>`);
                } else {
                  _push2(`<!---->`);
                }
                if (role.name != "owner") {
                  _push2(`<input type="checkbox"${ssrRenderAttr("value", role.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).selection) ? ssrLooseContain(unref(form).selection, role.id) : unref(form).selection) ? " checked" : ""} class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 sm:left-6 dark:bg-gray-700 dark:border-gray-700"${_scopeId}>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</td><td class="whitespace-nowrap py-4 pr-3 text-sm font-medium"${_scopeId}>${ssrInterpolate(role.name)}</td><td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 w-20"${_scopeId}>`);
                if (role.name != "admin") {
                  _push2(ssrRenderComponent(_sfc_main$7, {
                    ...
                    editStr: unref(T)("roles.edit", role.id)
                  ...
                  createVNode("div", { class: "bg-white dark:bg-gray-900 shadow sm:rounded-lg" }, [
                    createVNode("div", { class: "sm:rounded-lg" }, [
                      ...
                      createVNode("div", { class: "bg-gray-50 dark:bg-black/50 border-b dark:border-gray-700 sm:rounded-b-lg" }, [
                        props.roles && props.roles.data && props.roles.data.length ? (openBlock(), createBlock("div", { key: 0 }, [
                                ...
                                createVNode("div", { class: "relative overflow-hidden ring-1 ring-black ring-opacity-5" }, [
                                  ...
                                  createVNode("table", { class: "min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700" }, [
                                      ...
                                      createVNode("tr", null, [
                                        createVNode("th", {
                                          ...
                                          createVNode("input", {
                                            ...
                                            checked: indeterminate.value || unref(form).selection.length === props.roles.data.length,
                                            onChange: ($event) => unref(form).selection = $event.target.checked ? props.roles.data.map((p) => p.id) : [],
                                    ...
                                    createVNode("tbody", { class: "divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900" }, [
                                      (openBlock(true), createBlock(Fragment, null, renderList(props.roles.data, (role) => {
                                        return openBlock(), createBlock("tr", {
                                          ...
                                          createVNode("td", { class: "relative w-12 px-6 sm:w-16 sm:px-8" }, [
                                            unref(form).selection.includes(role.id) ? (openBlock(), createBlock("div", {
                                              key: 0,
                                              class: "absolute inset-y-0 left-0 w-0.5 bg-blue-600"
                                            })) : createCommentVNode("", true),
                                            role.name != "owner" ? withDirectives((openBlock(), createBlock("input", {
                                              key: 1,
                                              type: "checkbox",
                                              value: role.id,
                                          ...
                                          createVNode("td", { class: "whitespace-nowrap py-4 pr-3 text-sm font-medium" }, toDisplayString(role.name), 1),
                                          createVNode("td", { class: "whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 w-20" }, [
                                            role.name != "admin" ? (openBlock(), createBlock(_sfc_main$7, {
                                              key: 0,
                                              pm: "roles",
                                              row: role,
                                              property: "name",
                                              ...
                                              restoreFn: restoreRow,
                                              editStr: unref(T)("roles.edit", role.id)
                                            }, null, 8, ["row", "editStr"])) : createCommentVNode("", true)
                                          ])
                                        ], 2);
                                      }), 128))
                                    ])
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Role/List.vue");
